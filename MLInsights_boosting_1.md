...
multiple weak-classifiers are trained on distored versions of the
...
The main question Boosting trys to solve is how to find and combine
“weak-classifiers” having different knowleges from the training set in
order to form a final “strong classifier”? Those quesions cannot be
...
be highlighted in order to aquire “different knowledges” of it. Those
...
given a set of moves, so that one of them can be choosen at each round.
...
different knowleges of the data should be identified. Therefore, the
player responsible of providing a classifier will be rewarded if the
classifier has any knowlege about the given data, and the player
responsible of highlighting some aspects of the training set will be
rewarded if these aspects cannot be catched by the first player: this
would mean it found new knowlege to aquire. We see here that this goals
are conflicting: one player will try to emphisis the training samples
that are the hardest to classify, and the other will still try to find
some classifiers that overcome the difficulty. This is the description
of a simultaneously playing, zero-sum non-cooperative game. Moreover, if
we choose stumps as classifiers, it is proven that this infinite set can
be reduced to a finite number of stumps as its Vapnik-Chevonenkis
dimension is finite. Therefore, the game can also be considered to be
finite, and we then have at our disposal a rich theory to understand it.
...
“multiplicative-weight” algorithm for finite games. The follwing
...
trys to approximate this solution by iteratively updating one player’s
startegy given its performances against an omniscient opponent who knows
which strategy has been choosen before the game starts, and therefore
allways chooses the best strategy to maximize the loss of the first
...
picking the classifiers it the solution we are loooking for. The
following code shows that in action.
...
for(i in 1:games_number){
  ...
  h[[i]] <- rpart(income  ~ .,
                  ...
                  control = rpart.control(minsplit =1,
                                          minbucket=1, 
                                          cp=0, 
                                          maxdepth = 1)) # one depth tree are used here
...
through this adaptive mechanism.
...
sociodemographic informations. The following plots show the performance
...
### Acknowledgement
  ...
  - For further informartion about the used AdaBoost implementation
    please refere to <https://xgboost.readthedocs.io/en/>
